...
| ------ | -------------------- | ------- | ------------------------------------ | ------------------------------------------------------------------------------------------- |
| GET    | `/galon/current`     | —       | `{ giliran, namaAnggota, nudgedToday }` | Active turn (status `menunggu`), ordered `periodeMulai` asc limit 1. `nudgedToday` = caller sudah nudge hari ini WIB. |
| POST   | `/galon/:id/confirm` | —       | `{ next }`                           | Set `sudah_dibeli` + `confirmedAt`, rotate to next member (round-robin). No amount.         |
| POST   | `/galon/nudge`       | —       | `{ ok }`                             | Colek member yang giliran. **Maks 1x/hari WIB per member (locked 2026-08-12)** — ke-2 ditolak `409`. |
...
- **Nudge 1x/hari (locked 2026-08-12):** `Anggota.lastNudgeAt` diset saat nudge; jika sudah pernah hari WIB yang sama → `409 ConflictException`. Reset otomatis tengah malam WIB. Dashboard/galon expose `nudgedToday` agar tombol bel di-disable + note "udah colek hari ini".
...
- **When it's another member's turn:** a **bell/notif icon button** (36×36, bordered, bell glyph) — "colek"/nudge. Below it the hint text **"Galon habis? colek dia biar segera beli"**; after tapping, the button tints and the text becomes **"Kamu udah colek hari ini. Coba lagi besok."** (locked 2026-08-12: nudge max 1x/hari — state `nudgedToday` dari API, bukan local).
