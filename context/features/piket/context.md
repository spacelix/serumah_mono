...
| ------ | -------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------- | ------------------------------------------------------------------- |
| GET    | `/piket/today`       | —                                                                                    | `{ jadwal, ruangan[], jenisByRuangan, totalJenis, existingSubmission, nominalDenda }` | Full data for the Piket screen. `existingSubmission` includes `proofs` (foto + jenisSelesai) so the read-only view renders what was sent. `totalJenis` = active items for the proportional fine. |
