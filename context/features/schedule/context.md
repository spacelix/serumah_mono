...
- **No back-to-back weekend (fixed 2026-08-11):** `ensureWeekendWeek` membagi semua member di_kos MERATA 2 hari (Sabtu + Minggu) dengan rotasi `weekendOrdinal` agar adil lintas minggu (2 orang: Sabtu 1, Minggu 1; 3: Sabtu 2, Minggu 1). **Re-distribute penuh tiap pemanggilan** (hapus jadwal weekend tanpa submission lalu assign ulang semua di_kos) — jadi saat B di_kos setelah A, tetap Sabtu 1 / Minggu 1 (tidak menumpuk di Sabtu). Query weekendStatus scoped `status:'di_kos'` + rumah ini. **Jika hanya 1 orang di_kos, dia dapat SATU hari; hari lain Free**.
...
- **Pulang → weekday kembali (locked 2026-08-12):** saat user pilih `pulang`, jadwal weekend miliknya dihapus, lalu `restoreWeekdayForMember` **regenerate** hari piket yang kosong di minggu berjalan via `ensureWeekday` (round-robin, exclude member yang masih di_kos) — bukan mengisi semua slot kosong dengan satu orang (agar slot anggota lain tidak tertimpa).
