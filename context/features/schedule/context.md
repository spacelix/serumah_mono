...
- **No back-to-back weekend (fixed 2026-08-11):** `ensureWeekendWeek` membagi member di_kos ke weekend. Aturan (locked 2026-08-12): **pemilik slot weekday minggu ini** menentukan hari — pemilik hari piket pertama (Rabu) → Sabtu, pemilik kedua (Jumat) → Minggu (dihitung `weekdayOrdinal(day) % n`, deterministik). Non-owner (tidak kebagian weekday, mis. PJ) **tumpuk**: PJ → Sabtu bersama pemilik Rabu, berikutnya → Minggu bersama pemilik Jumat. Beberapa orang boleh di hari sama. **Jika hanya 1 di_kos (pemilik Jumat), dia dapat Minggu; hari lain Free.** Query weekendStatus scoped `status:'di_kos'` + rumah ini.
