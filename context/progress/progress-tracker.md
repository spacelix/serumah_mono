...
**Session 2026-08-18 (denda/iuran review fix) — approve denda ditolak 'tidak memiliki hak akses':** root cause = `@Roles('admin')` di `denda.controller.approve/reject` menolak reviewer non-admin SEBELUM service memvalidasi `reviewerId`. Denda milik PJ (Juan Akbar) di-assign reviewer ke Fadly (anggota) → Fadly ditolak guard. Fix: hapus `@Roles('admin')` dari approve/reject denda + `confirm-lunas` iuran (service sudah memvalidasi reviewerId). Docs: denda + iuran context (perbaikan locked 2026-08-18, review berbasis reviewer bukan role).

**Session 2026-08-18 (auto-fine cron bug fix) — Auto-fine tidak pernah jalan otomatis:** root cause = `runAutoFineCron` mengirim `new Date()` (jam 22:00) mentah ke `autoFineProcess` → `deadline(date)` (asumsi UTC-midnight) menghitung 20:00 WIB dari `date.getTime()+20h-7h` = ~jam berikutnya → `now <= deadline` selalu true → semua jadwal dianggap "masih on time", tidak pernah dibuat `bolong`+`Denda`. Terbukti: jadwal 14 & 17 Agu (Fadly, Juan Akbar) deadline lewat tanpa denda sampai di-trigger manual. Fix: cron pakai `this.toDate(new Date())`. Docs: schedule context.

**Session 2026-08-12 (profil darurat wajib) — No. telepon darurat + alamat wajib + reminder modal:** onboarding profile kini mewajibkan `kontakDarurat` + `alamat` (button "Lanjut" disabled + validasi alert, locked 2026-08-12). Dashboard expose `profileIncomplete` (true bila salah satu kosong) → Beranda menampilkan ConfirmDialog "Lengkapi profil darurat" ("Isi sekarang" → `/profile` / "Nanti") untuk user lama yang lewat onboarding sebelum field diwajibkan. `useProfileInvalidate` ikut invalidate dashboard agar reminder hilang setelah diisi. Docs: onboarding + dashboard context.

**Session 2026-08-12 (weekend/galon fixes) — Responsif + cooldown + nudge limit:** (1) **WeekendCard responsif fix** — mutation `useSetWeekendStatus` tak punya `onError` sehingga error server (403 frozen/400) senyap → tombol "mati". Kini onError → ConfirmDialog, tombol dapat pressed-state. (2) **Cooldown ganti status weekend 6 jam** (`WeekendStatus.updatedAt` + migrasi `weekend_cooldown_and_nudge`) — `setWeekendStatus` tolak `400` bila <6 jam sejak perubahan terakhir; dashboard expose `nextChangeAt` → tombol di-disable + hint "Ganti status lagi pukul HH:MM WIB". (3) **Nudge galon maks 1x/hari WIB** (`Anggota.lastNudgeAt`) — `POST /galon/nudge` tolak `409` bila sudah nudge hari itu; `currentFromAnggota`/dashboard expose `nudgedToday` → tombol bel di-disable + note "udah colek hari ini. Coba lagi besok." Docs: data-model, schedule/galon/dashboard context.

**Session 2026-08-12 — Jadwal manual bulanan (hapus cron):** TIDAK ada lagi cron jadwal (`pregenerateWeek`/`selfHealWeek`/`freezeWeekendCron` dihapus). Generate = manual bulanan oleh PJ (`generateRestOfWeek` → weekday hari ini → +1 bulan). Weekend event-driven (pilih di_kos → generate weekend minggu itu, dibagi merata + rotasi). Rule baru: di_kos → bebas weekday minggu itu TANPA digantikan siapa pun (`clearWeekdayForMember` hapus tanpa regenerate). Reminder akhir bulan → notif PJ. Empty state + tombol Generate saat minggu kosong. Auto-fine cron tetap.

**Session 2026-08-12 (schedule) — Fix weekday tidak berubah saat weekend status `pulang`:** root cause = `setWeekendStatus('pulang')` tidak menghapus baris `Jadwal` weekend milik anggota tersebut, sehingga `reconcileWeekdayForWeekend` masih menganggap anggota itu sebagai weekend assignee dan tidak mengembalikan weekday-nya ke pool. Fix: tambah `deleteMemberWeekendJadwal()` di `ScheduleService`, panggil saat status `pulang`, lalu `ensureWeekendWeek` untuk redistribute sisa di_kos + `reconcileWeekdayForWeekend`. Docs: `features/schedule/context.md`.
