...
**Log Viewer & Stats (context created 2026-08-07, build pending):** new feature `features/logviewer` registered in `build-plan.md` (under M6 production hardening). Locked decisions: sink = global `LoggerInterceptor` → Postgres `LogEntry` table; access = self-hosted `GET /admin` HTML page in `apps/api`; auth = fully public (`@Public`, user-approved risk); no mobile involvement. `data-model.md` updated with `LogEntry` (not rumah-scoped, exception).

> **Note:** M5 is still in progress — this feature was added at the user's request. Awaits approval to be built; will run through the same docs → verify → commit flow, one commit per item.
