...
**Last completed:** M3.1 Ruangan & Jenis Piket — new `RuanganModule` (`/ruangan` CRUD + reorder + jenis piket), admin-gated via `@Roles('admin')` + shared `RumahScopeService` (single-PJ invariant), all queries rumah-scoped. `turbo run build lint typecheck test` green.

**Next:** M3.2 Schedule (round-robin, weekend, freeze, generate).
